...
from s_enformer.attention_modules import bigbird as attention_module_bigbird
from s_enformer.attention_modules import enformer as attention_module_enformer

CHANNELS = 1536
BIN_SIZE = 128
SEQUENCE_LENGTH = CHANNELS * BIN_SIZE
TARGET_LENGTH = 896
...
class Enformer(snt.Module):
        ...
        transformer = Sequential(lambda: [
            Sequential(lambda: [
                Residual(Sequential(lambda: [
                    ...
                    # attention_module_enformer.MultiheadAttention(
                    #     **whole_attention_kwargs, name=f'attention_{i}'),
                    attention_module_bigbird.MultiheadAttention(
                        **whole_attention_kwargs, name=f'attention_{i}'),
    ...
    @tf.function()
    def train(self, inputs: tf.Tensor) -> Dict[str, tf.Tensor]:

        trunk_embedding = self.trunk(inputs, is_training=True)
        return {
            head: head_module(trunk_embedding, is_training=True)
            for head, head_module in self.heads.items()
        }
...
class Sequential(snt.Module):
    ...
    def __init__(self,
                 layers: Optional[Union[
                     Callable[[], Iterable[snt.Module]],
...
def pooling_module(kind, pool_size):
    ...
    if kind == 'attention':
        return SoftmaxPooling1D(
            pool_size=pool_size, per_channel=True, w_init_scale=2.0
        )
    elif kind == 'max':
